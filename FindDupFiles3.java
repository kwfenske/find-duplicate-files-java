/*
  ...
  DOS/Windows or the "cmp" command on Linux.  What to do with files is the
  ...
  See the DeleteDupFiles Java application to delete duplicate files when there
  is a "known good" folder and a folder of unknown files.  See CompareFolders
  for comparing two folders to determine if all files and subfolders are
  identical.  See FileChecksum to generate or test checksums for a single file.

  Apache License or GNU General Public License
  --------------------------------------------
  FindDupFiles3 is free software and has been released under the terms and
...
public class FindDupFiles3
{
  ...
  static final String PROGRAM_TITLE = "Find Duplicate Files With "
    + DIGEST_NAME + " Checksums - by: Keith Fenske";
...
//  }
    ...
    outputText = new JTextArea(16, 40);
    ...
    outputText.setText(
      "\nFind duplicate files with the same size and the same " + DIGEST_NAME
      + " checksum."
...
*/
  static void processFileData()
  {
    ...
    TreeMap sizeEntry;            // one entry from <sizeMap>
    ...
    if (cancelFlag == false)      // do nothing more if user cancelled
      putError("Comparing file sizes and " + DIGEST_NAME + " checksums.");
    ...
    sizeKeySet = sizeMap.keySet(); // get sorted list of file sizes
    ...
    while ((!cancelFlag) && sizeIterate.hasNext()) // any more file sizes?
    {
      ...
      sizeEntry = (TreeMap) sizeMap.get(sizeKey);
...
*/
  static void processUnknownFile(File givenFile)
  {
    ...
    if (zeroFlag || (fileSize > 0)) // should we ignore empty files?
    {
      ...
      if (sizeMap.containsKey(sizeKey) == false)
                                // is there an entry for this file size?
        sizeMap.put(sizeKey, new TreeMap());
                                // no entry, create an empty TreeMap
      ((TreeMap) sizeMap.get(sizeKey)).put(filePath, givenFile);
